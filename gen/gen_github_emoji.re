...
let url_re = {
  open Re;
  let code_point = group(rep1(alnum) |> rep1);
  seq([
    unicode_url,
    code_point,
...
let parse_png_url = url => {
  let* matches = Re.exec_opt(url_re, url);
  Some(Re.Group.all(matches) |> Array.to_list |> List.tl);
};

exception Invalid_png_url(string);

exception Invalid_entry(Yojson.Safe.t);
...
let make_entry = ((name, `String(png_url))) =>
  switch (parse_png_url(png_url)) {
  | None => None
  | Some([x, ""]) =>
    Some(
      Emoji.{
        code_points: [str_to_point(x)],
        string: str_to_unicode(x),
        name,
      },
    )
  | Some([x, y]) =>
    Some(
      Emoji.{
        code_points: [
          str_to_point(x),
          Emoji.zwj_code_point,
          str_to_point(y),
        ],
        string:
          String.concat(
            "",
            [str_to_unicode(x), Emoji.zwj, str_to_unicode(y)],
          ),
        name,
      },
    )
...
exception Invalid_response(Yojson.Safe.t);

let make_table: Yojson.Safe.t => Emoji.table = (
  fun
  | `Assoc(fields) =>
    fields |> List.map(validate_entry) |> List.filter_map(make_entry)
  | json => raise(Invalid_response(json)):
    Yojson.Safe.t => Emoji.table
);

let run = arg => {
  let decl = {
    let table_str = fetch_json() |> make_table |> Emoji.show_table;
    "let v = " ++ table_str;
  };
...
let () = {
  open Kwdcmd;
  let conv = Kwdcmd.Arg.conv(~docv="OUT_FILE", Fpath.(of_string, pp));
  Exec.run(
