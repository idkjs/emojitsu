...
let () =
  Exec.select(
    ...
    [
      ...
      {
        let+ unicode = Required.pos("UNICODE", ~conv=Arg.string, ~nth=0, ());
      ...
      {
        let+ name = Required.pos("EMOJI_NAME", ~conv=Arg.string, ~nth=0, ());
        Lib.Github.lookup_unicode(name);
      },
      cmd(
        ~name="emojify",
        ~doc=
          "Replace all names of the form :emoji_name: with the corresponding unicode",
      ) @@
      {
        let+ file =
          Optional.pos(
